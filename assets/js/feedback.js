jQuery(document).ready(function ($) {
    ...
    $('#add_feedback input, #add_feedback textarea').on('blur', function () {
        $('#add_feedback input, #add_feedback textarea, .art_left_name, .art_left_tel').removeClass('error');
    ...
    var options = {
        ...
        success: function (request, xhr, status, error) {
            ...
            if (request.success === true) {
                ...
                $('#submit-feedback').val('Отправлено!');
            } else {
                // Если поля не заполнены, выводим сообщения и меняем надпись на кнопке
                $.each(request.data, function (key, val) {
                    $('.art_left_' + key).addClass('error');
                    $('.art_right_' + key).addClass('error');
                    $('.art_right_' + key).before('<span class="error-' + key + '">' + val + '</span>');
                });
        ...
        error: function (request, status, error) {
            $('#submit-feedback').val('Что-то не так...');
        }
    };
    // Отправка формы
